...
これは`<chat>`タグの挙動仕様である「LLMに送る文章全体のコンテキストを使用せず、chatタグで囲まれたタグのテキストだけを使用する」
...
代わりに、現状のtagwritingの推奨する方法を教えます。
 ...
 2. その新規ファイルを利用したいファイルへ`<include>newfile.md</include>`と記述する。

これは、基本的にテキストファイル単体をLLMへのコンテキストとして扱うというtagwritingの設計思想を反映しています。よりコンテキストを適切に制御したい場合は、別テキストを用意するというのが、スマートかつシンプルな方法であり、粒度もより整うと考えれます。