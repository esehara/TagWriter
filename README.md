...
## 🏷️ 属性エイリアス（attr alias）の使い方

TagWritingでは、タグの属性（例: `<prompt:funny>...</prompt>` の `funny` 部分）にエイリアス（説明文や指示）を付与できます。

### 1. YAMLテンプレートに属性エイリアスを定義

`sample.yaml` などのテンプレートで `attrs` セクションを追加します。
...
指定した属性に対応する説明文（エイリアス）が、LLMへ送るプロンプトの`Rule`欄に自動で追加されます。
