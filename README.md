...
#### FAQ: なぜ<prompt></prompt>は展開されないのですか？

これは単純な設計意図として、`include`が単純な文章結合ではなく、あくまで最新のドキュメントを別ファイルで別々に管理するための方法であると考えられます。処理として、`<include></include>`タグは、`<prompt>`実行後もテキストにタグを保持したまま残り続けますが（置換されませんが）、もしこのinclude先の`<prompt></prompt>`が実行されると仮定した場合、この`prompt`の内容はどこに挟まるのかが不明確です。このため、`include`先の`<prompt></prompt>`は実行されません。
...
## chatタグで個別にWikipediaの記事を参照したんだけど、できるようにしませんか？

これは<chat>タグの挙動仕様である「テキストファイル全体のコンテキストを使用せず、chatタグで囲まれたタグのテキストだけを使用する」
を曖昧にする可能性が高くユーザーの混乱を招く可能性があります。また開発者個人として見積もっても、多少複雑な実装が必要になると考えられるため、現状ではやりません（優先度が低いです）。

代わりに、現状のtagwritingの推奨する方法を教えます。

 1. 一度新規ファイルを作成し、そこで<wikipedia>と<prompt>を使って文章を生成する。
 2. その新規ファイルを利用したいファイルへ<include>newfile.md</include>と記述する。（あるいはコピーアンドペーストを行う）

これは、「一つのテキストファイル全体を、基本的なLLMへのコンテキストとして扱う」というtagwritingの設計思想を反映しています。よりコンテキストを適切に制御したい場合は、別テキストを用意するというのが、スマートかつシンプルな方法であり、またこのように文章を分割することで、テキストファイルの編集自体も容易になると考えられます。